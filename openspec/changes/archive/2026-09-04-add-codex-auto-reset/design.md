...
- Every successful terminal transition atomically stores its one-shot audit notice in `autoreset.json` in the same coordinator-locked write that clears pending state and sets the success cooldown. If that write fails, the previously persisted pending attempt remains for an idempotent retry. The footer also drains the separately locked `autoreset-notices.json` queue when present; neither notice path can overwrite pending consume state.
