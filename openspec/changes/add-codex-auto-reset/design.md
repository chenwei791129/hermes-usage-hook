...
- One-shot audit notices live in a separate `autoreset-notices.json` protected by `autoreset-notices.lock/`; notice queue/pop operations cannot overwrite pending consume state. If that lock is unavailable after a successful preflight reset, a fallback notice is atomically retained in `autoreset.json` under the already-held coordinator lock and drained later under the same lock.
